...
function displayBooks(books) {
  ...
  likeButtons.forEach(function (button) {
    button.addEventListener('click', function () {
      let bookId = this.closest('.book').getAttribute('data-id');
...
function displayTopPicks(books) {
  let topPicksList = document.getElementById('top-picks-list');
  let topPicks = books.filter(function (book) {
...
function populateGenres(books) {
  let genres = books.map(function (book) {
    return book.genre; 
  });
...
function filterBooks() {
  ...
  xhr.onload = function () {
    if (xhr.status >= 200 && xhr.status < 300) {
      let books = JSON.parse(xhr.responseText);
      let filteredBooks = genre ? books.filter(function (book) {
